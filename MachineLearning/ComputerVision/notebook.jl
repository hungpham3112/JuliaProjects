...
# ╔═╡ c8ad7850-0aee-11ee-15f8-2be3997130d8
using TestImages, ImageCorners, ImageShow, Images, ImageView, ImageFiltering

# ╔═╡ 8aa23c3e-eac0-40c0-a712-9dce519fc53e
img = testimage("cameraman")

# ╔═╡ 8f6d3712-6595-4e37-bc0d-df1b1514937f
corners = ImageCorners.imcorner(img; method = ImageCorners.harris);

# ╔═╡ 10711a2d-86dc-4926-a3ea-e7559b50b1de
imshow(corners);

# ╔═╡ a9290991-390d-464f-b204-4cf1cd1b4282
begin

function harris_corner_detector(image; k=0.04, threshold=0.01)
    # Convert the image to grayscale
    gray = Gray.(image)
    
    # Compute the gradients using finite differences
    # Define gradient kernels
    ∇x_kernel = centered([-1 0 1])
    ∇y_kernel = centered([-1 0 1]')
    
    # Compute the gradients using imfilter
    ∇x = imfilter(gray, ∇x_kernel)
    ∇y = imfilter(gray, ∇y_kernel)
    # Compute the products of gradients
    ∇x2 = ∇x .^ 2
    ∇y2 = ∇y .^ 2
    ∇xy = ∇x .* ∇y
    
    # Apply Gaussian filter to the products of gradients
	σ = 2.0
	∇x2_padded = padarray(∇x2, (1, 1), pad=0)
    ∇y2_padded = padarray(∇y2, (1, 1), pad=0)
    ∇xy_padded = padarray(∇xy, (1, 1), pad=0)
    
    ∇x2_smoothed = imfilter(∇x2_padded, KernelFactors.gaussian(σ))
    ∇y2_smoothed = imfilter(∇y2_padded, KernelFactors.gaussian(σ))
    ∇xy_smoothed = imfilter(∇xy_padded, KernelFactors.gaussian(σ))
    
    # Compute the corner response function
    detM = ∇x2_smoothed .* ∇y2_smoothed - ∇xy_smoothed .^ 2
    traceM = ∇x2_smoothed + ∇y2_smoothed
    response = detM - k * traceM .^ 2
    
    # Threshold the response
    response[response .< threshold * maximum(response)] .= 0
    
    # Find corner coordinates
    corner_coords = findall(x -> x != 0, response)
    
    # Create Corner objects for corners
    keypoints = [Corner(coord[2], coord[1], 1.0) for coord in corner_coords]
...
keypoints = harris_corner_detector(img)

# Draw the detected corners on the image
output = drawkeypoints(image, keypoints, color=RGB(0, 1, 0))
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "2e7bd5b44c5575fa6b06dbdd306fb5a5fb17bde7"
...
[[deps.ImageCorners]]
deps = ["ColorTypes", "ColorVectorSpace", "ImageCore", "ImageFiltering", "PrecompileTools", "StaticArrays", "StatsBase"]
git-tree-sha1 = "5a6540e3c3155bd864fb7a0e7692efd909399cc6"
