...
# ╔═╡ f00a6aa0-538e-11ee-0f2e-ebe5e3578198
using LinearAlgebra, HDF5, Images , StatsPlots , ImageShow, ImageView, StatsBase, PlutoUI

# ╔═╡ 4035ba60-c727-40e7-b517-f2fad989f13d
TableOfContents()

# ╔═╡ d407cc7e-ca9a-4466-b503-f16bbf120735
md"""
# Data preprocessing
"""

# ╔═╡ fcb30895-a8be-488f-8460-d3b478448091
begin
	train_data_path = "./datasets/train_catvnoncat.h5"
	test_data_path = "./datasets/test_catvnoncat.h5"
end

# ╔═╡ b4446b41-2961-4b0e-bd49-4479bfb890d1
train_data = h5open(train_data_path, "r")

# ╔═╡ 09a4c867-987d-4f00-8410-16ad76d63bef
test_data = h5open(test_data_path, "r")

# ╔═╡ c2e799bb-cb5b-4139-b5c5-8db24cea1411
begin
	XTrain = read(train_data, "train_set_x");
	yTrain = read(train_data, "train_set_y")';
	XTest = read(test_data, "test_set_x");
	yTest = read(test_data, "test_set_y")';
end

# ╔═╡ b5371aca-a5a1-49ed-98df-065d9d5529e2
md"""
## Display image
"""

# ╔═╡ 7a3cbc89-cc29-40f5-8809-26a8195a26ca
function convert_to_image(dataset)
	img_perm = colorview(RGB{N0f8}, dataset)
	return permutedims(img_perm, (2, 1, 3))
end

# ╔═╡ 1753bb49-3004-4b66-b329-16e1ebc13091
XTrain_image, XTest_image = convert_to_image.([XTrain, XTest])

# ╔═╡ e894df6c-728d-4ef7-8eb9-8cc44245f6be
mosaicview(XTrain_image[:, :, 1:20], nrow=4, ncol=5)

# ╔═╡ f867a20e-d649-4cd3-b13d-24814ec1d0e7
mosaicview(XTest_image[:, :, 1:20], nrow=4, ncol=5)
...
begin
	println("""
	XTrain size $(size(XTrain))
	yTrain size $(size(yTrain))
	XTest size $(size(XTest))
	yTest size $(size(yTest))
	#images = $((size(XTrain)[4]))
	image size $(size(XTrain)[2:3])
	""")
end
...
begin
	XTrain_new = Float64.(reshape(XTrain, (12288, 209)))
	XTest_new = Float64.(reshape(XTest, (12288, 50)))
	println("train_set_x_flatten shape: $(size(XTrain_new))")
	println("train_set_y shape: $(size(yTrain))")
	println("test_set_x_flatten shape: $(size(XTest_new))")
	println("test_set_y shape: $(size(yTest))")
end
...
@bind data Select([(XTrain_new, "XTrain") => "XTrain", 
				   (XTest_new, "XTest") => "XTest"])

# ╔═╡ 77e10422-4f6a-4762-b064-ee8e8b68b4f6
density(data[1][:, :],
		legend=false,
		title="Original $(data[2])"
)

# ╔═╡ 13add70e-9cde-4062-b776-ee0f99059cd4
@bind normalized_data Select([(normalized_XTrain, "Normalized XTrain") => "Normalized XTrain", (normalized_XTest, "Normalized XTest")  => "Normalized XTest"])

# ╔═╡ 6b429821-175e-4a0f-b144-537eb88516f4
density(normalized_data[1],
		legend=false,
		title="$(normalized_data[2])"
)

# ╔═╡ 3894d1cc-6899-4701-8b9a-eec7694e29a0
md"""
...
$(Resource("https://goodboychan.github.io/images/LogReg_kiank.png"))
...
**Key steps**:
In this step, you will carry out the following steps:
...
# ╔═╡ f03beb2d-fe51-4776-92c0-56fa0385baff
md"""
...
__Note__: Style architecture `FCNN` see [here](https://github.com/alexlenail/NN-SVG)
"""

# ╔═╡ c356e129-89ad-4977-9575-98c223bb1b71
md"""
## Define constant
"""

# ╔═╡ cfadfbde-a307-4e35-a4ea-b36bf7bbdb52
m = size(normalized_XTrain, 2)

# ╔═╡ 62625677-2160-4425-b45e-3bb984449330
md"""
## Weight and bias initialization
"""

# ╔═╡ 4033f202-2f0c-44aa-8a53-f6b6890ea7e8
function initialize_parameters(size)
	return zeros(size), 0.0
end

# ╔═╡ 0b0cc3ac-6edc-4695-a3ad-c929e0b81a8c
md"""
## Sigmoid function
...
# ╔═╡ 866a3273-cce3-4fc7-a018-096f5ab48748
function σ(z)
	return 1.0 / (1.0 + exp(-z))
end

# ╔═╡ 0e74e651-79c0-4acb-8bbf-1ad0b9b5cd6c
md"""
## Loss function
...
# ╔═╡ 7f233af3-a393-4a8c-8ffe-a5fcaaf26df5
function 𝐋(a, y)
	return - y * log10(a) - (1.0 - y) * log10(1.0 - a)
end

# ╔═╡ 937fee36-ca68-4b28-a92c-8f29aedb8821
md"""
## Cost function
...
# ╔═╡ bcfcf96d-724d-482b-bb11-f1b59f5df5ee
function 𝐉(A, Y)
	return (1.0 / m) * sum(𝐋.(A, Y))
end

# ╔═╡ b5472e0b-eacf-4355-b975-9811c95cbcee
md"""
## Back propagation
...
# ╔═╡ 9e253182-9585-4fbb-9b19-3aec545b58e1
function back_propagation(X, A, Y)
	dW = (1 / m) * X * (A - Y)'
	db = (1 / m) * sum(A - Y)
	return dW, db
end

# ╔═╡ 1799d8f9-5af2-4bfd-9b2a-b7072f93c183
md"""
## Optimization (Gradient descent)
"""

# ╔═╡ 856b4286-bc76-4e13-bf6c-3857058289e1
function gradient_descent(W, X, A, b, Y, α=0.01)
	dW, db = back_propagation(X, A, Y)
	W -= α * dW
	b -= α * db
	return W, b
end

# ╔═╡ bebb443c-3525-48ae-a2fa-90a9a4ade0f5
function predict(W, X, b)
    A = W'X .+ b
    prediction = ifelse.(A .< 0.5, 0, 1)
    return prediction
end

# ╔═╡ 37566052-6a05-4af2-8877-1b592eeb5867
function model(XTrain, YTrain, XTest, YTest; α=0.01, epochs=2500, print_cost=false)
	costs = []
	W, b = initialize_parameters(size(XTrain, 1))
	for i in 0:epochs
		Z = W'XTrain .+ b
		A = σ.(Z)
		cost = 𝐉(A, YTrain)
		if i % 100 == 0
			push!(costs, cost)
			print_cost && println("Epoch: $i, cost: $cost")
		end
		W, b = gradient_descent(W, XTrain, A, b, YTrain, α)
	end
	YTrain_prediction = predict(W, XTrain, b)
	YTest_prediction = predict(W, XTest, b)
	println("Train accuracy: ", sum(YTrain_prediction .== YTrain) / length(YTrain))
	println("Test accuracy: ", sum(YTest_prediction .== YTest) / length(YTest))
	return costs, YTrain_prediction, YTest_prediction
end

# ╔═╡ d2217ffc-7254-4ced-bf55-a0061b8eb702
md"""
# Run models with different learning rate
"""

# ╔═╡ 37da292e-e7ad-48b9-a381-24595cd62d33
logistic_regression_model1 = model(normalized_XTrain, yTrain, normalized_XTest, yTest; α=0.01, epochs=2500, print_cost=true)

# ╔═╡ 0f64b488-45f5-4e70-82be-d83604079046
logistic_regression_model2 = model(normalized_XTrain, yTrain, normalized_XTest, yTest; α=0.001, epochs=2500, print_cost=true)

# ╔═╡ 52a62616-b480-4aea-95a3-3d85200926c9
logistic_regression_model3 = model(normalized_XTrain, yTrain, normalized_XTest, yTest; α=0.0001, epochs=2500, print_cost=true)

# ╔═╡ 9386aca9-90fb-425f-b48a-fe5e7abddadd
begin
	plot(logistic_regression_model1[1],
		label="α=0.01",
	...
	plot!(logistic_regression_model3[1],
		label="α=0.0001"
	)
end
